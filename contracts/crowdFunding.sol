...
event Action (uint id, string actionTYpe, address indexed executor, uint timestamp);
...
function createProject(string memory title, string memory description, string memory imageURL, uint cost, uint expiresAt) public returns (bool) {
    require(bytes(title).length > 0, "title cannot be less than zero");
    require(bytes(description).length > 0, "description cannot be empty");
    require(bytes(imageURL).length > 0, "ImageURL cannot be empty");
    require(cost > 0 ether, "cost cannot be empty");
...
function deleteProject(uint id) public returns (bool) {
    require(projects[id].status == statusEnum.OPEN, "Project no longer exist");
    require(msg.sender == projects[id].owner, "Unauthorized Entity");
...
function requestRefund (uint id) public returns (bool) {
    require(
        projects[id].status != statusEnum.REVERTED || 
        projects[id].status != statusEnum.DELETED, "Project marked as revert or deleted"
    );
